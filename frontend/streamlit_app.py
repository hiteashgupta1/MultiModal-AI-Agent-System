...
os.environ["HF_TOKEN"] = "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"
...
# Initialize a basic user file if it doesn't exist
...
# ---------------------------
# Dynamically change colors based on toggle
if "theme" not in st.session_state:
...
if not st.session_state.logged_in:
    ...
    # Navigation Buttons 
        ...
        # Create dynamic columns based on the number of tabs
        nav_cols = st.columns(len(tabs))
    
        for i, tab_name in enumerate(tabs):
        # Map tab names to icons for the UI
            icons = {
...
# ---------------------------
    ...
    with center_col:
        ...
        # Content Split: Input on Left, Tools on Right
            ...
            if result:
                ...
                if result.get("agent") == "vision":
                ...
                elif result.get("agent") == "image_gen":
                    ...
                    if selected_lang_code != st.session_state.current_lang:
                        ...
                        # Request the updated audio from the backend using the new language
                        try:
                            # We reuse the exact same text input, passing the newly chosen language
                            updated_payload = {
                                "text": user_input,
                                "lang": selected_lang_code
                            }
                            
                            # Call your backend router again to generate the correct language audio
                    ...
                    with col1:
        # Pulls 'confidence' from backend
                        conf = payload.get("confidence", 0.0)
                        st.metric("Confidence Score", f"{float(conf) * 100:.1f}%")

                    with col2:
        # Pulls 'hallucinated' and 'hallucination_ratio'
                    ...
                    with col3:
        # Pulls 'bertscore_f1' from backend
                        f1_score = payload.get("bertscore_f1", 0.0)
                        st.metric("Accuracy", f"{float(f1_score) * 100:.1f}%")

        # DEEP EVALUATION (ROUGE & BERTScore)
                    c1, c2, c3 = st.columns(3)
                    with c1:
                        st.write(f"Precision: `{payload.get('bertscore_precision', 0.0):.4f}`")

                    with c2:

                        st.write(f"Recall: `{payload.get('bertscore_recall', 0.0):.4f}`")

                    with c3:

                        st.write(f"F1 Score: `{payload.get('bertscore_f1', 0.0):.4f}`")
                    ...
                    # 1. Initialize chat history and state
                    ...
                    if question :
                        ...
                        if not original_doc:
                            ...
                            with st.spinner("RAG Agent searching document..."):
                                try:
                                    import requests
                                    # Replace with your actual FastAPI URL if deployed
    ...
    # --- NEW PROFESSIONAL CONTENT BELOW ---
            st.markdown("<br><br>", unsafe_allow_html=True)
            st.markdown("---")
        
        # 1. CORE PILLARS (Feature Grid)
...
# ---------------------------
        ...
        # 3. ABOUT & CONTACT SECTION
            ...
            # Centered Container for vertical flow
            ...
            # 2. CORE TECHNOLOGY (Vertical Stack)
                st.markdown(f"""
                    <div class="glass-card" style="margin-bottom: 25px;">
...
# ---------------------------
    with st.sidebar:
        ...
        if os.path.exists(FILE):
            ...
            if len(df) > 0:
                st.metric("System Confidence", f"{round(df['confidence'].mean() *100, 2)}%")
